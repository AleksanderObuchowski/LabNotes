...
export type ProjectStats = { notes: number; verified: number; experiments: number };
...
export async function statsForProjects(projectIds: string[]): Promise<ProjectStats> {
  if (projectIds.length === 0) return { notes: 0, verified: 0, experiments: 0 };
  await ensureSchema();
...
// Compact, token-efficient markdown of a project's findings — the default read
// path for LLM agents (avoids the key-repetition cost of raw JSON).
export function buildDigest(repoFullName: string, notes: Note[]): string {
  const lines: string[] = [`# LabNotes — ${repoFullName}`, ""];
  if (notes.length === 0) {
    lines.push("_No findings logged yet._");
    return lines.join("\n");
  }
  ...
  for (const [experiment, group] of groups) {
    ...
    for (const n of group) {
      ...
      lines.push(`- ${metric}${n.title} _(${flag}, ${sha}, ${date}, @${n.author_login})_`);
    }
    lines.push("");
  }
  return lines.join("\n").trimEnd() + "\n";
